...
export function AppointmentCard({ appointment: a, onStatusChange, canAdmin, token }: Props) {
  ...
  const hasConsultation = !!a.consultation;
    ...
    <>
      {showAttendModal && (
        <AttendAppointmentModal
          appointment={a}
          token={token}
          onDone={() => { setShowAttendModal(false); onStatusChange(a.id, 'DONE'); }}
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-2 flex-wrap">
              ...
              <div className="flex items-center gap-1.5">
                {hasConsultation && (
                  <span className="text-xs bg-vet-100 text-vet-700 px-2 py-0.5 rounded-full font-medium flex items-center gap-1">
            ...
            {/* Materials section (for completed appointments) */}
            {canAddMaterials && a.status === 'DONE' && (
                    ...
                    {!showAddForm ? (
                      ...
                      <div className="bg-white border border-vet-100 rounded-lg p-3 space-y-2 mt-2">
                        ...
                        <div className="flex gap-2">
                          <input
                            type="number" min={1} max={selectedProduct?.currentStock ?? 999} value={addForm.quantity}
